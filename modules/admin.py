...
def load():
    ...
    async def blacklist_group(event):
        ...
        if group_id not in blacklist:
            ...
            await helpers.temp(event, helpers.wm("🚫 Grup ini masuk blacklist broadcast."))
        else:
            await helpers.temp(event, helpers.wm("🚫 Grup ini sudah di blacklist."))
        await event.delete()
    ...
    async def unblacklist_group(event):
        ...
        if group_id in blacklist:
            ...
            store.save("blacklist", blacklist)
            await helpers.temp(event, helpers.wm("✅ Grup dihapus dari blacklist."))
        else:
            await helpers.temp(event, helpers.wm("ℹ️ Grup ini tidak ada di blacklist."))
        await event.delete()
    ...
    async def show_blacklist(event):
        blacklist = store.load("blacklist", [])
        if not blacklist:
            await helpers.temp(event, helpers.wm("📭 Blacklist kosong."))
            await event.delete()
            return
        lines = []
    ...
    async def ban(event):
        ...
        if not reply or not reply.sender:
            await helpers.temp(event, helpers.wm("❌ Reply ke pesan user yang mau di-ban."))
            await event.delete()
            return
        uid = reply.sender_id
        try:
            ...
            await helpers.temp(event, helpers.wm(f"🔨 Banned `{uid}`"))
        except Exception as e:
            await helpers.temp(event, helpers.wm(f"❌ Gagal ban: {e}"))
        await event.delete()
    ...
    async def unban(event):
        reply = await event.get_reply_message()
        if not reply or not reply.sender:
            await helpers.temp(event, helpers.wm("❌ Reply ke pesan user yang mau di-unban."))
            await event.delete()
            return
        uid = reply.sender_id
        try:
            ...
            await client(EditBannedRequest(event.chat_id, uid, rights))
            await helpers.temp(event, helpers.wm(f"✅ Unbanned `{uid}`"))
        except Exception as e:
            await helpers.temp(event, helpers.wm(f"❌ Gagal unban: {e}"))
        await event.delete()
    ...
    async def kick(event):
        reply = await event.get_reply_message()
        if not reply or not reply.sender:
            await helpers.temp(event, helpers.wm("❌ Reply ke pesan user yang mau di-kick."))
            await event.delete()
            return
        uid = reply.sender_id
        try:
            ...
            rights = ChatBannedRights()
            await client(EditBannedRequest(event.chat_id, uid, rights))
            await helpers.temp(event, helpers.wm(f"🦵 Kicked `{uid}`"))
        except Exception as e:
            await helpers.temp(event, helpers.wm(f"❌ Gagal kick: {e}"))
        await event.delete()
    ...
    async def promote(event):
        reply = await event.get_reply_message()
        if not reply or not reply.sender:
            await helpers.temp(event, helpers.wm("❌ Reply ke pesan user yang mau di-promote."))
            await event.delete()
            return
        uid = reply.sender_id
        try:
            ...
            await helpers.temp(event, helpers.wm(f"👑 Promoted `{uid}`"))
        except Exception as e:
            await helpers.temp(event, helpers.wm(f"❌ Gagal promote: {e}"))
        await event.delete()
    ...
    async def demote(event):
        reply = await event.get_reply_message()
        if not reply or not reply.sender:
            await helpers.temp(event, helpers.wm("❌ Reply ke pesan user yang mau di-demote."))
            await event.delete()
            return
        uid = reply.sender_id
        try:
            ...
            await helpers.temp(event, helpers.wm(f"⬇️ Demoted `{uid}`"))
        except Exception as e:
            await helpers.temp(event, helpers.wm(f"❌ Gagal demote: {e}"))
        await event.delete()